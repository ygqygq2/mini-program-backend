...
class WxAuth {
   ...
   */
  getAuthorizeURL(redirect_uri: any, scope: any, state: any) {
  ...
  * 处理token，更新过期时间
  */
  processToken (data: { create_at: number; openid: any }) {
   ...
   * }
   * ```
   * @param {any} options 传入openid或者参见Options
   */
  async getUser(options: any) {
    ...
    if (token.isValid()) {
      ...
      accessToken = (newToken as any).data.session_key;
    }
    console.log("accessToken", accessToken);
    return await this._getUser(openid, accessToken);
  }

  _getUser (openid: string, accessToken: any, lang?: any) {
    return new Promise((resolve, reject) => {
 ...
 * @param {String} code 授权获取到的code
 */
  async getUserByCode (code: any) {
    const token = await this.getAccessToken(code);
    return await this.getUser((token as any).data.openid);
  }
}
